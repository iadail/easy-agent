...
from easy_agent.tools import ToolHandler
...
class RuntimePluginHost:
    ...
    def _resolve_entry_point(self, source: str) -> RuntimePlugin:
        ...
        if hasattr(entry_points, "select"):
            matches = list(entry_points.select(group="easy_agent.plugins", name=source))
        else:
            legacy_entry_points = cast(dict[str, list[Any]], entry_points)
            matches = [item for item in legacy_entry_points.get("easy_agent.plugins", []) if item.name == source]
        if not matches:
