...
class DirectSandboxRunner(BaseSandboxRunner):
    def prepare(self, request: SandboxRequest) -> PreparedSubprocess:
        return PreparedSubprocess(command=request.command, cwd=request.cwd, env={**os.environ, **request.env})
    ...
    def run(self, request: SandboxRequest) -> SandboxResult:
        ...
        return SandboxResult(stdout=result.stdout.strip(), stderr=result.stderr.strip(), returncode=result.returncode)
...
class WindowsSandboxRunner(BaseSandboxRunner):
    ...
    def prepare(self, request: SandboxRequest) -> PreparedSubprocess:
        raise RuntimeError('Windows Sandbox does not expose a direct subprocess launch context')
...
class SandboxManager:
    ...
    def prepare(self, request: SandboxRequest) -> PreparedSubprocess:
        return self._resolve_runner(request.target).prepare(request)
