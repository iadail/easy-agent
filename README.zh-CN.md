...
| 定向 Python 手工验证 | 仓库内脚本写出 `.easy-agent/manual-verify/20260330-github-fed/manual-verification-report.json` | passed |
...
这轮手工 Python 验证覆盖了 GitHub automation helpers、optional local skill 加载、optional-only 参数超集下的 duplicate-call suppression、tau history grounding helpers，以及 `run_remote()` 前的 federation 自动发现。

### 真实网络矩阵

| 场景 | 传输 | 状态 | 耗时 (s) | 说明 |
| --- | --- | --- | --- | --- |
| `cross_process_federation` | `http_poll` | passed | `0.7481` | 跨进程 well-known discovery 与 send/poll 联邦链路通过 |
| `live_model_federation_roundtrip` | `http_poll` | skipped | `6.9997` | live-model loopback 行已真正进入矩阵，但本次会话里 provider 连接失败，远端 agent 尚未返回结果 |
| `disconnect_retry_chaos` | `http_webhook` | passed | `4.1800` | callback 重试、`pushNotificationConfig`、`sendSubscribe` 与 resubscribe 链路通过 |
| `duplicate_delivery_replay_resilience` | `http_webhook` | passed | `3.6405` | 重复投递与 replay 读取保持了稳定的 federated task event log |
| `workbench_reuse_process` | `local_process` | passed | `2.3959` | process workbench 成功复用同一个长生命周期 session root |
| `workbench_reuse_container` | `podman_exec` | skipped | `3.4967` | 当前会话无法读取宿主机 Podman 身份凭据 |
| `workbench_incremental_snapshot_reuse_container` | `podman_exec` | skipped | `3.4215` | 当前会话无法读取宿主机 Podman 身份凭据 |
| `workbench_reuse_microvm` | `podman_machine_ssh` | skipped | `0.1807` | 当前宿主机的 podman-machine 锁文件或身份路径不可访问 |
| `workbench_incremental_snapshot_reuse_microvm` | `podman_machine_ssh` | skipped | `0.1905` | 当前宿主机的 podman-machine 锁文件或身份路径不可访问 |
| `replay_resume_failure_injection` | `sqlite_checkpoint` | passed | `9.0014` | 注入失败后的 resume、replay、fork 恢复链路通过 |

汇总：`5 passed`、`0 failed`、`5 skipped`。
来源：`.easy-agent/real-network-report.json`，生成时间 `2026-03-30T07:29:01Z`。
...
- 继续把 federation card 与 task 协商贴近最新公开 A2A 面，补齐更丰富的 artifact / part 级模态声明、`ListTasks` / `ListTaskEvents` 的 cursor 分页（`pageToken` / `nextPageToken`），以及围绕 push delivery 的 notification compatibility metadata。
- 把联邦安全继续推进到生产级远端信任模型，包括更强的 auth scheme hints、callback 签名校验、OAuth / OIDC 流程、callback audience 校验，以及可选的 agent server 之间 mTLS。
- 把 MCP 能力协商从当前的 roots 支持继续推进到完整的 `roots/list_changed` 流程，并加强 `streamable_http` 下的断线重连、鉴权刷新和 server-initiated lifecycle 处理。
- 把 MCP sampling 与 elicitation 从当前 text-first bridge 继续扩展到更丰富的结构化内容块和 form payload 保真能力，让低风险请求在 provider 与 runtime 支持时保留更多结构，同时继续把高风险远程请求放在 human approval 之后。
...
- 把这轮新加进 real-network 矩阵但仍为 `skipped` 的行稳定推进到 `passed`，方法包括预检 live-model 连通性、Podman 身份路径可达性，以及 container / microVM 的增量快照 warm cache。
