...
这份快照合并了 2026 年 3 月 30 日重新完成的 Python 侧验证结果和 live public-eval 刷新结果，以及保留自 2026 年 3 月 29 日的 real-network 矩阵和保留自 2026 年 3 月 27 日的 benchmark 工件。3 月 30 日这轮验证使用了 Python `3.12.11`、本地 `.env.local` 凭据、真实 DeepSeek 调用，以及仓库当前的真实 MCP 集成测试。
...
| 单元测试 | `.\.venv\Scripts\python.exe -m pytest tests/unit -q --basetemp=%TEMP%\easy-agent-pytest\unit-full-<timestamp>` | `83 passed` |
| 真实集成测试 | `.\.venv\Scripts\python.exe -m pytest tests/integration -m real -q --basetemp=%TEMP%\easy-agent-pytest\integration-full-<timestamp>` | `5 passed` |
| Live benchmark 工件 | `.easy-agent/benchmark-report.json` | 复用 2026-03-27 的现有快照 |
| Live public-eval 刷新 | 调用 `run_public_eval_suite('easy-agent.yml')` 的 Python 辅助脚本 | 已刷新并得到 `overall.bfcl_pass_rate = 0.8750` |
| Live real-network 工件 | `.easy-agent/real-network-report.json` | 复用 2026-03-29 的现有快照 |
| Python CLI 覆盖 | full unit suite 内的 `CliRunner` CLI 测试 | 已包含在当前 `83 passed` 结果内 |
...
来源：`.easy-agent/public-eval-report.json`，已于 2026-03-30 刷新。

当前注意事项：

- live suite 结束后仍会出现 Windows `asyncio` 子进程清理 warning，但真实测试和报告刷新都已完成。
- 当前剩余的 BFCL 失败已经变成行为层面的重复或过度工具调用（`simple_5`、`multiple_7`、`parallel_multiple_3`），不再是 provider-side schema 拒绝。
- 当前剩余的 tau2 失败主要集中在带历史上下文的 `update_task_with_message_history`，模型仍会追问，而不是直接锚定到已有 tool result。
- public-eval 报告现在会记录 `fallback_stage` 和 `fallback_attempts`，方便后续回归时排查 provider-compatible 的降级路径。

## 下一步补强

这些下一步补强方向是根据当前公开的 A2A 与 MCP 协议面整理出来的，而不是只来自仓库内部 backlog。

- 继续把 federation card 与 task 协商贴近最新公开 A2A 面，补齐更丰富的 artifact / part 级模态声明、`ListTasks` 的 cursor 分页（`pageToken` / `nextPageToken`），以及更清晰的 notification compatibility metadata。
...
- 把 MCP sampling 从当前 text-first bridge 继续扩展到更丰富的内容块保真能力，让低风险请求在 provider 与 runtime 支持时保留更多结构，同时继续把高风险请求放在 human approval 之后。
- 增加带阶段感知的 public-eval 分析、按 provider 划分的 schema 兼容矩阵，以及针对 duplicate-call suppression 和 history-grounding 的更强回归用例。
